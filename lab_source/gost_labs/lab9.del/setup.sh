...
ip netns exec ns1 ip link add h1 type veth peer name h1-br
ip netns exec ns1 ip link set h1-br master br0
ip netns exec ns1 ip link set h1 up
ip netns exec ns1 ip link set h1-br up

ip netns exec ns1 ip link add h2 type veth peer name h2-br
ip netns exec ns1 ip link set h2-br master br0
ip netns exec ns1 ip link set h2 up
ip netns exec ns1 ip link set h2-br up

ip netns exec ns1 ip addr add 10.0.0.1/24 dev h1
ip netns exec ns1 ip addr add 10.0.0.2/24 dev h2

ip netns exec ns1 ping -I h1 10.0.0.2
...
ip netns exec ns1 ip link add veth6 type veth peer name veth2 netns ns1
ip netns exec ns1 ip addr add 10.0.20.3/24 dev veth6
...
ip netns add ns3
ip netns exec ns1 ip link add veth5 type veth peer name veth4 netns ns3
ip netns exec ns1 ip addr add 10.0.30.2/24 dev veth5
ip netns exec ns1 ip link set veth5 up
ip netns exec ns3 ip addr add 10.0.30.1/24 dev veth4
ip netns exec ns3 ip link set veth4 up
# ip netns exec ns3 ip link set lo up
# ip netns exec ns3 ip route add default via 10.0.30.2 dev veth4
...
ip netns exec ns1 iptables -t nat -A POSTROUTING -o veth5 -j MASQUERADE
